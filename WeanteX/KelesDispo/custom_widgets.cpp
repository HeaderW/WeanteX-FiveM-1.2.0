...
#include <string>
    ...
    bool Checkbox(const char* label, bool* v)
    {
        ...
        const float oval_width = 36;
        const float oval_height = 16;
        const ImVec2 pos = window->DC.CursorPos;

        const float w = GetContentRegionMax().x - style.WindowPadding.x;

        const ImRect total_bb(pos + ImVec2(w - oval_width, 0), pos + ImVec2(w, 32));
        ...
        ImVec4 bg_color = *v ? ImVec4(0.8f, 0.2f, 0.2f, 1.0f) : ImVec4(0.25f, 0.25f, 0.25f, 1.0f);
        it_anim->second.background = ImLerp(it_anim->second.background, bg_color, g.IO.DeltaTime * 6.f);
        ...
        const ImVec2 checkbox_pos = pos + ImVec2(w - oval_width, (32 - oval_height) / 2);
        const ImRect checkbox_rect(checkbox_pos, checkbox_pos + ImVec2(oval_width, oval_height));


        GetWindowDrawList()->AddRectFilled(checkbox_rect.Min, checkbox_rect.Max, GetColorU32(it_anim->second.background), oval_height / 2.0f);


        float circle_radius = (oval_height / 2.0f) * 0.75f;
        float circle_travel = oval_width - (circle_radius * 2) - 4;
        float circle_x = checkbox_rect.Min.x + circle_radius + 2 + (circle_travel * it_anim->second.alpha_mark);
        float circle_y = checkbox_rect.GetCenter().y;


        ImVec4 circle_color = *v ? ImVec4(1.0f, 0.4f, 0.4f, 1.0f) : ImVec4(0.9f, 0.9f, 0.9f, 1.0f);
        GetWindowDrawList()->AddCircleFilled(ImVec2(circle_x, circle_y), circle_radius, GetColorU32(circle_color));
    ...
    bool CheckboxClicked(const char* label, bool* v)
    {
        ...
        const float oval_width = 36;
        const float oval_height = 16;
        const ImRect bb(pos + ImVec2(GetContentRegionMax().x - oval_width - 20, 0), pos + ImVec2(GetContentRegionMax().x - 20, 32));
        ...
        const ImVec2 checkbox_pos = pos + ImVec2(GetContentRegionMax().x - oval_width - 20, (32 - oval_height) / 2);
        const ImRect checkbox_rect(checkbox_pos, checkbox_pos + ImVec2(oval_width, oval_height));
    ...
    bool ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
    {
        ...
        if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
        {
            ...
            GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, (32 - CalcTextSize(label).y) / 2), GetColorU32(c::text::text), label);
    ...
    bool ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
    {
        ...
        float bars_width = 209.f;
        float sv_picker_size = ImMax(bars_width * 1, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)) + 0;
        float sv_bar_size = 20;
        ...
        sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size), picker_pos.x, picker_pos.x + sv_picker_size - 2);
