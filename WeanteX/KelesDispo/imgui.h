...
namespace ImGui
{
    ...
    IMGUI_API ImU32         GetColorU32(const ImVec4& col, float alpha_mul = 1.f);
...
enum ImDrawFlags_
{
    ...
    ImDrawFlags_ShadowCutOutShapeBackground = 1 << 9,
};
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left, float rounding = 0, ImDrawFlags flags = 0);
...
struct ImFontAtlasShadowTexConfig
{
    int     TexCornerSize;
    int     TexEdgeSize;
    float   TexFalloffPower;
    float   TexDistanceFieldOffset;
    bool    TexBlur;

    inline ImFontAtlasShadowTexConfig() { memset(this, 0, sizeof(*this)); }
    IMGUI_API void SetupDefaults();
    int     GetRectTexPadding() const   { return 2; }
    int     CalcRectTexSize() const     { return TexCornerSize + TexEdgeSize + GetRectTexPadding(); }
    int     GetConvexTexPadding() const { return 8; }
    int     CalcConvexTexWidth() const;
    int     CalcConvexTexHeight() const;
};
...
struct ImFontAtlas
{
    ...
    int                         ShadowRectIds[2];
    ImVec4                      ShadowRectUvs[10];
    ImFontAtlasShadowTexConfig  ShadowTexConfig;
